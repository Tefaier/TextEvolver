...
'''
def find_in_clean(clean_words: list, look_for_words: list, to_find: str, new_words_num: int, mutations: bool, replace_rules: dict): # time eater - try to solve
    words_num = len(look_for_words)
    if clean_words == [''] or look_for_words == [''] or new_words_num == 0 or len(clean_words) < words_num:
        return {"found": False}
    results = {"found": False}
    start_location = None
    for i in range(
            (len(clean_words) + 1 - words_num - new_words_num) if (len(clean_words) + 1 - words_num - new_words_num >= 0) else 0,
            len(clean_words) + 1 - words_num):
        word_chunk = ' '.join(clean_words[i:i + words_num])
        word_borders = re.split(to_find, word_chunk, flags=re.IGNORECASE)
        if len(word_borders) != 1 and word_borders[0] == '':
            if ((word_borders[1] == '') or (mutations and (word_borders[1] in possible_mutations))):
                ...
                start_location = i
                break
    if results["found"] and replace_rules != None:
        results.update({"replace_map": [[x, x] for x in clean_words]})
        replacer = replace_rules["replace_with"].split(' ')
        replacer = ['' if i >= len(replacer) else (' '.join(replacer[i:]) if (i == words_num - 1) else replacer[i]) for
                    i in range(0, words_num)]
        for index in range(start_location, start_location + words_num):
            if results["replace_map"][index][1][0].islower():
                replacer[index - start_location] = replacer[index - start_location][0].lower() + replacer[index - start_location][1:]
            else:
                replacer[index - start_location] = replacer[index - start_location][0].upper() + replacer[index - start_location][1:]
            results["replace_map"][index][1] = replacer[index - start_location]
        if replace_rules["units"] != None:
            from main import is_float
            digit_version = text2int(" ".join([x[1] for x in results["replace_map"]]))
            for i in range(0, len(results["replace_map"])):
                results["replace_map"][i][1] = digit_version[i]
            digit_found = False
            for index in range(start_location - 1, -1, -1):
                if is_float(results["replace_map"][index][1]):
                    digit_found = True
                    results["replace_map"][index][1] = str(
                        round(float(results["replace_map"][index][1]) * replace_rules["units"], 1))
                elif results["replace_map"][index][1] in ["of", '', 'to', 'or', 'so']: # 'or' and 'so' added for special cases
                    pass
                else:
                    if not digit_found:
                        if not replace_rules["word_and_not_value"]:
                            results["replace_map"][start_location][1] = str(replace_rules["units"]) + ((" " + results["replace_map"][start_location][1]) if results["replace_map"][start_location][1]!='' else '')
                        else:
                            results["found"] = False
                            #return results
                    break
